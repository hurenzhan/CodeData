import apiReport from '@/web-design/api/reportDetails'
...
export default {
  data() {
    return {
      ...
      pageArray: [], // 记录所有页面中全选信息, 每个页面对应一个数组,若本页全选, 数组中存放所有全选指标, 否则为空
      // -----------table-------------------
      tableInfo: {
        radio: false,
        radioKey: 1,
  ...
  methods: {
    ...
    async queryInfo() {
      ...
      if (resData.statusCode === '0') {
        ...
        resData.data.metricsInfos.forEach((v) => {
          let unitNum = ''
    ...
    async querybusiScope() {
      this.$refs.dataTable.judgeType()
      const res = await apiReport.querybusiScope()
      if (!res) return
      this.business = res
    },
    // 百川主题
    async querySubject() {
      ...
      if (businessId) {
        const res = await apiReport.querySubject({
          params: {
            businessId
          }
        })
    ...
    async queryTime() {
      this.$refs.dataTable.judgeType()
      const res = await apiReport.queryTime()
      if (!res) return
      this.time = res
    },
    // 时间粒change
    ...
    async queryDimList() {
      // 获取是否勾选只显示权限内指标
      const res = await apiReport.queryDimList({
        params: {
          rightFlag: 0
        }
      })
    ...
    selectChange() {
      ...
      if (this.pageArray[this.currentPage - 1].length === dataTableNum) {
